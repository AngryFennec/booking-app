...
def write_tables_list(tables):
    """Function for showing list of tables with checkboxes"""
    st.write("")
    col11, col12, _ = st.columns([0.05, 0.8, 0.15])
    for i, table in enumerate(tables):
        done = col11.checkbox("", table.checked, key=f"table{table.id}")
        if done:
            table.checked = True
...
class PageManagement(AbstractPage):
    ...
    def show_page(self):
        """Implemented method for showing management page"""
        ...
        # Show buttons for manage tables list
        ...
        # Show fields for creating new table
        ...
        if st.session_state.is_similar_tables:
            st.error("Tables with similar names are not allowed")
